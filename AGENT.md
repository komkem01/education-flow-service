# AI Agent Instructions - education-flow Storage Service
...
func New(conf *config.Config[Config], dependencies...) *Module {
    tracer := otel.Tracer("education-flow.service_name.module_name")
...
```go
import "log/slog"
import "education-flow/internal/log"
...
```go
import "education-flow/internal/provider"
...
```bash
# Application
APP_NAME=education-flow-storage-service
